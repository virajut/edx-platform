...
def find_fixme(options):
    ...
    for system in systems:
        ...
        cmd = (
            ...
            u"--output-format=parseable {apps} "
            u"> {report_dir}/pylint_fixme.report".format(
                apps=apps_list,
    ...
    for system in systems:
        ...
        if clean or not system_report.exists():
            sh(
                ...
                u"> {report_dir}/pylint.report".format(
                    flags=" ".join(flags),
                    apps=apps_list,
                    report_dir=report_dir
                ),
...
def run_pylint(options):
    ...
    if num_violations < lower_violations_limit > -1:
        fail_quality(
            ...
            u"Expected to see at least {lower_limit} pylint violations. "
    ...
    if num_violations > upper_violations_limit > -1:
        fail_quality(
            ...
            u"The limit is {upper_limit}.".format(upper_limit=upper_violations_limit)
        )
    else:
...
def run_eslint(options):
    ...
    sh(
        ...
        u"--ext .js --ext .jsx --format=compact . | tee {eslint_report}".format(
...
def run_xsslint(options):
    ...
    if error_message:
        fail_quality(
            ...
            u"See {xsslint_report} or run the following command to hone in on the problem:\n"
...
def run_xsscommitlint():
    ...
    if num_violations:
        fail_quality(
            'xsscommitlint',
            u"FAILURE: XSSCommitLinter Failed.\n{error_message}\n"
            u"See {xsscommitlint_report} or run the following command to hone in on the problem:\n"
            "  ./scripts/xss-commit-linter.sh -h".format(
...
def run_diff_quality(
    ...
    try:
        sh(
            ...
            u"{reports} {percentage_string} {compare_branch_string} "
            u"--html-report {dquality_dir}/diff_quality_{type}.html ".format(
