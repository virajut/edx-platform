...
class TestPopulateExpiryDate(MockS3Mixin, TestCase):
    ...
    def test_recent_approved_verification(self):
        ...
        # Check that expiry_date for only on verification is set
        ...
        expiry_date = SoftwareSecurePhotoVerification.objects.get(
            updated_at=outdated_verification.updated_at
        ).expiry_date
        self.assertEqual(expiry_date, None)
    ...
    def test_approved_verification_expiry_date(self):
        ...
        approved_verification = SoftwareSecurePhotoVerification.objects.get(user_id=approved_verification.user_id,
                                                                            status='approved')
