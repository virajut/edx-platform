...
class TestSites(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_paid_course_registration(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn(u"Enroll in {}".format(self.course_with_visibility.id.course), resp.content)
        self.assertNotIn(u"Add {} to Cart ($10)".format(self.course_with_visibility.id.course), resp.content)

        # now try on the site
        url = reverse('about_course', args=[text_type(self.course_with_visibility.id)])
        resp = self.client.get(url, HTTP_HOST=settings.MICROSITE_TEST_HOSTNAME)
        self.assertEqual(resp.status_code, 200)
        self.assertNotIn(u"Enroll in {}".format(self.course_with_visibility.id.course), resp.content)
