...
def add_coupon(request, course_id):
    ...
    try:
        ...
        coupon.save()
        return JsonResponse(
            {u'message': _(u"coupon with the coupon code ({code}) added successfully").format(code=code)}
        )

    if coupon:
        return JsonResponse(
            {u'message': _(u"coupon with the coupon code ({code}) already exists for this course").format(code=code)},
            status=400)  # status code 400: Bad Request
...
def update_coupon(request, course_id):  # pylint: disable=unused-argument
    ...
    try:
        ...
        return JsonResponse(
            {u'message': _(u"coupon with the coupon id ({coupon_id}) DoesNotExist").format(coupon_id=coupon_id)},
            status=400)  # status code 400: Bad Request
    ...
    coupon.save()
    return JsonResponse(
        {u'message': _(u"coupon with the coupon id ({coupon_id}) updated Successfully").format(coupon_id=coupon_id)}
    )
...
def get_coupon_info(request, course_id):  # pylint: disable=unused-argument
    ...
    if not coupon.is_active:
        return JsonResponse({
            u'message': _(u"coupon with the coupon id ({coupon_id}) is already inactive").format(coupon_id=coupon_id)
        }, status=400)  # status code 400: Bad Request
